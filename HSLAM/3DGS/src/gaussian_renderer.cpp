...
#include "3DGS/include/gaussian_renderer.h"



/**
 * @brief Static function that renders 3DGS scenes
 * 
 * Calls the 3DGS CUDA code through a series of wrappers
 * 1. GaussianRenderer::render() in gaussian_renderer.cpp <- We are here
 * 2. GaussianRasterizer::forward() in gaussian_rasterizer
 * 3. RasterizeGaussiansCUDA() in rasterize_points.cu
 * 4. Functions in the cuda_rasterizer libarary
 * 
    ...
    bool use_override_color)
{
    ...
    auto screenspace_points_densify = torch::zeros_like(pc->getXYZ(),
        torch::TensorOptions().dtype(pc->getXYZ().dtype()).requires_grad(true).device(torch::kCUDA));
    ...
    auto means2D_densify = screenspace_points_densify;
    ...
    auto [rendered_image, radii] = rasterizer.forward(
        means3D,
        means2D,
        means2D_densify,
        opacity,
