...
ImGuiViewer::ImGuiViewer(
    HSLAM::IOWrap::ImguiBrigeViewer* GSviewer_,      // gd -gui -27july2026
    std::shared_ptr<HSLAM::GaussianMapper> pGausMapper, // gd -gui -27july2026
    bool training)
    : glfw_window_width_(1600),
      ...
      record_render_frames_(false)
{
    this->pGausMapper_ = pGausMapper;

    result_dir_= pGausMapper->getResultDir();
    ...
    std::filesystem::path image_dir = result_dir_ / "viewer_images";
    if (record_render_frames_)
        CHECK_DIRECTORY_AND_CREATE_IF_NOT_EXISTS(image_dir);

    // Create drawers
    if (GSviewer_)
    {
        pMapDrawer_ = std::make_shared<ImGuiMapDrawer>();
    }
}
...
void ImGuiViewer::readConfigFromFile(std::filesystem::path cfg_path)
{
    ...
    lambda_isoreg_ = pGausMapper_->lambdaIsoReg();
    ...
    record_render_frames_ = pGausMapper_->recordRenderFrames();
...
void ImGuiViewer::run()
{
    ...
    // Main loop
    while(!viewer_isStopped() && !glfwWindowShouldClose(window) && GSviewer->running)
    {
        ...
        if (GSviewer)
        {
            ...
            {
                Sophus::SE3f initTwc = GSviewer->camToWorld;
            ...
            {
                pMapDrawer_->SetCurrentCameraTwc(GSviewer->camToWorld);
            }
        ...
        if (GSviewer && GSviewer->haveLiveImage)
        {
            //--------------Draw SLAM frame image--------------
            cv::Mat SLAM_img_to_show;
            cv::Mat SLAM_img_input = GSviewer->liveImage;
            
            if (SLAM_image_viewer_scale_ != 1.0f)
            {
                int width = rendered_image_width_;
                int height = static_cast<int>(SLAM_img_input.rows * SLAM_image_viewer_scale_);
                cv::resize(SLAM_img_input, SLAM_img_input, cv::Size(width, height));
            ...
            cv::Rect SLAM_image_rect(0, 0, SLAM_img_input.cols, SLAM_img_input.rows);
            SLAM_img_input.copyTo(SLAM_img_to_show(SLAM_image_rect));
            cv::cvtColor(SLAM_img_to_show, SLAM_img_to_show, cv::COLOR_BGR2RGB /* gd -gui -27july2026: OpenCV 4 dropped the CV_ aliases */);
            ...
            if(record_render_frames_&& tracking_vision_&&GSviewer){
                    total_render_frames++;
                    std::filesystem::path image_dir = result_dir_ / "viewer_images";
                    cv::Mat image_cv;
                    cv::cvtColor(rendered_img, image_cv, cv::COLOR_RGB2BGR /* gd -gui -27july2026: OpenCV 4 dropped the CV_ aliases */);
                    image_cv.convertTo(image_cv, CV_8UC3, 255.0f);
                    cv::imwrite(image_dir / (std::to_string(total_render_frames) + ".jpg"), image_cv);
            }
        ...
        if (show_main_rendered_)
        {
            ...
            else
            {
                ...
                if(record_render_frames_){
                    ...
                    std::filesystem::path image_dir = result_dir_ / "viewer_images";
                    cv::Mat image_cv;
                    cv::cvtColor(main_img, image_cv, cv::COLOR_RGB2BGR /* gd -gui -27july2026: OpenCV 4 dropped the CV_ aliases */);
                    image_cv.convertTo(image_cv, CV_8UC3, 255.0f);
                    cv::imwrite(image_dir / (std::to_string(total_render_frames) + ".jpg"), image_cv);
                }
            }
        ...
        lambda_isoreg_ = params_in.lambda_isoreg;
        ...
        ImGui::SetNextWindowSize(ImVec2(panel_width_, display_panel_height_), ImGuiCond_Once);
        {
            ...
            if (training_)
            {
                if (GSviewer) ImGui::Checkbox("Tracking vision", &tracking_vision_);
            }
        ...
        if (training_)
        {
            ...
            ImGui::SetNextWindowSize(ImVec2(panel_width_, training_panel_height_), ImGuiCond_Once);
            {
                ...
                ImGui::SliderFloat("Lambda IsoReg", &lambda_isoreg_, 0.01f, 10.0f, "%.2f");
        ...
        params_out.lambda_isoreg = lambda_isoreg_;
        ...
        params_out.do_inactive_geo_densify = do_inactive_geo_densify_; // gd -gui -27july2026
...
bool ImGuiViewer::viewer_isStopped()
{
    std::unique_lock<std::mutex> lock_viewer_status(viewer_mutex_status_);
    return this->stopped_;
}

void ImGuiViewer::viewer_signalStop(const bool going_to_stop)
{
    std::unique_lock<std::mutex> lock_viewer_status(viewer_mutex_status_);
